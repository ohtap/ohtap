...
# EDIT THIS SECTION #
collection = "All-Transcripts-Plain" # path to your collections folder
...
def create_and_rename(filename, new_filename):
	...
	if os.path.exists(full_new_filename):
		if filename.split(".")[0].split("_")[-1].isalpha():
			print("Suppressed_Alpha")
		elif filename.split(".")[0].split("_")[-1].isnumeric():
			print("Suppressed_Numeric")
			full_new_filename = os.path.join(current_dir, new_collection, filename)
		else:
...
for filename in sorted(os.listdir(collection)):
	...
	if new_filename not in all_files:
		newnamelist = new_filename.split('_')
		if newnamelist[-1] == "multi.txt":
			print("Suppressed_Multi")
			new_filename = newnamelist[0]
			for i in range(1, len(newnamelist)-1): new_filename += "_" + newnamelist[i]
			new_filename += ".txt"
		else:
