...
# Static folders where data is going to be located!
corpus_folder = './data/corpus-files'
...
# TODO: Change metadata input to be not static (i.e. different versions of the files)
def read_arguments(session):
	curr_corpus = corpus_folder + "/" + sys.argv[2]
	curr_keyword_list = session['keyword-lists'][sys.argv[3]]

	return curr_corpus, curr_keyword_list, metadata_file

# Makes a name for the current run based on keyword list, collection name, and date
# TODO: Pass in dates to make it consistent!
def create_new_name(directory, words):
	collection_name = directory.split("/")[-1]
	keywords_name = words['name']
	now = datetime.datetime.now()
	date = "{}{}{}{}{}".format(now.year, now.month, now.day, now.hour, now.minute)

	name = "{}_{}_{}".format(collection_name, keywords_name, date)

	# TODO: Check if already existing, but shouldn't since it's the date.
...
def read_keywords(words, report_name):
	include = words['include']
	exclude = words['exclude']
	all_words = include + exclude
	include_regexes = []
	exclude_regexes = []
	...
	write_span_line(report_name, "Keyword List Name", words['name']),
	write_span_line(report_name, "Total number of keywords", len(include_regexes) + len(exclude_regexes)),
	write_span_line(report_name, "Keywords to include", ", ".join(include)),
	write_span_line(report_name, "Keywords to exclude", ", ".join(exclude))

	return all_words, include_regexes, exclude_regexes
...
def set_up(session):
	directory, words, metadata = read_arguments(session)
	include = words['include']
	exclude = words['exclude']
	name = create_new_name(directory, words)
	subcorpora_dirname = name
	report_name = "{}/{}_report.html".format(data_results_folder, name)
...
def get_top_words(filenames, content, name):
	...
	stats_name = "{}/{}_file_top_words.csv".format(data_results_folder, name)
...
# Reads in the session.json file
# TODO: Add validation
...
def write_subcorpora(subcorpora_dirname, filenames, content, keyword_freq_files):
	os.mkdir("{}/{}".format(data_results_folder, subcorpora_dirname))
	for i in range(len(filenames)):
		file = filenames[i]
		if file not in keyword_freq_files: continue
		new_file = "{}/{}/{}".format(data_results_folder, subcorpora_dirname, file)
...
def find_keywords(files_for_inclusion, filenames, content, words, include_regexes, exclude_regexes, report_name, name, subcorpora_dirname):
	...
	keyword_counts_name = "{}/{}_keyword_counts.csv".format(data_results_folder, name)

	# Stores the frequency of each keyword by file (filename --> keyword --> count)
	keyword_freq_files = defaultdict(lambda:0)
	keyword_counts_file_name = "{}/{}_keyword_counts_by_file.csv".format(data_results_folder, name)

	# Stores keyword collocations
	keyword_collocations = defaultdict(lambda:0)
	keyword_collocations_name = "{}/{}_keyword_collocations.csv".format(data_results_folder, name)
	keyword_collocations_format_name = "{}/{}_keyword_collocations_formats.csv".format(data_results_folder, name)
	...
	multiple_keyword_name = "{}/{}_multiple_keywords.csv".format(data_results_folder, name)

	# Stores data on keyword formats
	keyword_formats = defaultdict(lambda:[])
	keyword_format_name = "{}/{}_keyword_formats.csv".format(data_results_folder, name)
...
def main():
	...
	directory, all_words, files_for_inclusion, name, subcorpora_dirname, report_name, filenames, content, words, include_regexes, exclude_regexes, metadata = set_up(session)
	get_top_words(filenames, content, name)
	find_keywords(files_for_inclusion, filenames, content, all_words, include_regexes, exclude_regexes, report_name, name, subcorpora_dirname)
