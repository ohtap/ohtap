...
#Given the DataFrame of all interviewees, produce a list of collection ids
def get_all_collection_ids(df_v):
	collection_ids = set()

	for i, r in df_v.iterrows():
		if r["collection_id"] not in collection_ids:
			collection_ids.add(r["collection_id"])
...
def write_metadata_to_csv(df_i, df_v, all_contexts, report_name):
	new_metadata = []
	metadata_header = ["project_file_name", "interview_id", "collection_id", "keyword in context", "keyword", "false hit", "notes",
					   ...
					   "researcher_assumed_birth_decade", "interviewee_birth_city", "interviewee_birth_state", "interviewee_birth_country",
					   "birthplace_type", "place_of_significant_residence", "sex", "identified_race", "researcher_assumed_race",
					   "education", "current_or_most_recent_occupation", "eeo-1_job_title"]
	...
	for i, r in df_i.iterrows(): # For collections with metadata
		...
		if file in all_contexts.keys():
			...
			researcher_assumed_birth_decade = interviewee_row["researcher_assumed_birth_decade"]
			...
			for context in contexts:
				...
				for keyword in keywords:
					...
					entry = [project_file_name, interview_id, collection_id, keyword_in_context, keyword, false_hit, notes,
							 ...
							 researcher_assumed_birth_decade, interviewee_birth_city, interviewee_birth_state, interviewee_birth_country,
							 birthplace_type, place_of_significant_residence, sex, identified_race, researcher_assumed_race, education,
...
def write_subcorpora(subcorpora_dirname, filenames, content, keyword_freq_files, collection_ids):
...
def set_up():
	...
	collection_ids = get_all_collection_ids(df_v)
	...
	return directory, words_arg, files_for_inclusion, name, subcorpora_dirname, report_name, filenames, content, words, include_regexes, \
		   exclude_regexes, df_i, df_v, collection_ids
...
def find_keywords(df_i, df_v, files_for_inclusion, filenames, content, words, include_regexes, exclude_regexes, report_name, name,
				  subcorpora_dirname, collection_ids, save_subcorpora):
	...
	if save_subcorpora: # Save the files with keywords and merge their metadata into one csv file
		write_metadata_to_csv(df_i, df_v, all_contexts, report_name)
		write_subcorpora(subcorpora_dirname, filenames, content, keyword_freq_files, collection_ids)
...
def main():
	...
	exclude_regexes, df_i, df_v, collection_ids = set_up()

	"""
	Has not proven particularly useful, takes a long time to run,
	and only produces different results with changes to the corpus; unchanged between runs
	...
	find_keywords(df_i, df_v, files_for_inclusion, filenames, content, words, include_regexes, exclude_regexes, report_name,
				  name, subcorpora_dirname, collection_ids, True)
